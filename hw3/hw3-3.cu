...
__global__ void phase3_kernel(int* d_Dist, int n, int Round, int start) {
    int half = BS >> 1;
    int p = Round * BS;
    int il = threadIdx.y;
    int jl = threadIdx.x;
    int blk_x = blockIdx.x + start;
...
#pragma omp parallel num_threads(2)
{
    ...
    int pix_start = begin_id * n * BS;
    int chunk = round / 2;
    if (id) chunk += round % 2;
    size_t size = n * n * sizeof(int);
    size_t chunk_size = chunk * n * BS * sizeof(int);
    cudaSetDevice(id);
    cudaMalloc(&d_Dist[id], size);
    cudaMemcpy(d_Dist[id] + pix_start, Dist + pix_start, chunk_size, cudaMemcpyHostToDevice);
    #pragma omp barrier
    dim3 block_dim(TS, TS);
    dim3 grid_dim_p2(round - 1, 2);
    dim3 grid_dim_p3(chunk, round - 1);
    ...
    cudaMemcpy(Dist + pix_start, d_Dist[id] + pix_start, chunk_size, cudaMemcpyDeviceToHost);
    cudaFree(d_Dist[id]);
}
}
