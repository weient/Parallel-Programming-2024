...
void LinearLayer(float *A, float *B, float *C, float *D, int n, int k, int m) {
    #pragma acc parallel loop tile(32, 32) copyin(A[0:n*k], B[0:k*m], C[0:m]) copyout(D[0:n*m])
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            float sum = C[j];
            for (int a = 0; a < k; a++) {
...
void Sigmoid(float *A, int n, int m) {
    #pragma acc parallel loop tile(32, 32) copy(A[0:n*m])
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            // Sigmoid(x) = 1/(1+exp(x))
...
void Argmax(float *A, int *D, int n, int m) {
    #pragma acc parallel loop copyin(A[0:n*m]) copyout(D[0:n])
    for (int i = 0; i < n; i++) {
        float mx = A[i * m];
        int index = 0;
        for (int j = 1; j < m; j++) {
